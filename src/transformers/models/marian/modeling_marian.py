...
from .configuration_marian import MarianTopicConfig
...
@add_start_docstrings(
    "The bare Marian Model with topic embeddings outputting raw hidden-states without any specific head on top.", MARIAN_START_DOCSTRING
)
class MarianTopicModel(MarianPreTrainedModel):
...
@add_start_docstrings(
    "The Marian Topic Model with a language modeling head. Can be used for summarization.", MARIAN_START_DOCSTRING
)
class MarianTopicMTModel(MarianPreTrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: MarianTopicConfig):
        super().__init__(config)
        self.model = MarianTopicModel(config)
